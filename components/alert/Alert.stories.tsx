...
export const Actionable: Story = {
  ...
  play: async ({ canvas, userEvent, args }) => {
    await userEvent.click(
      canvas.getByRole('button', { name: args.actionLabel as string }),
    );
    await expect(args.onAction).toHaveBeenCalledOnce();
    // Alert remains visible after the action — it is not auto-dismissed.
    await expect(canvas.queryByText(args.message)).toBeInTheDocument();
  },
};
...
/**
 * After dismissal, focus moves to the next focusable element that follows the
 * alert in the DOM — keeping keyboard navigation predictable.
 */
export const FocusRestoredAfterDismiss: Story = {
  parameters: {
  ...
  play: async ({ canvas, userEvent }) => {
    ...
    // Wait for the requestAnimationFrame that moves focus after the alert unmounts.
    await new Promise((resolve) => requestAnimationFrame(resolve));
    await expect(canvas.getByTestId('next-target')).toHaveFocus();
  },
};
