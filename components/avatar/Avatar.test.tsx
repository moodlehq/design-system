...
describe('Avatar: Unit Tests', () => {
  ...
  it('forwards aria-label to the root element', () => {
    // aria-label is spread onto the root <span> via ...props. The component no
    // longer carries role="img" (switched to <img alt> pattern), so query by
    // attribute rather than accessible role.
  ...
  it('applies alt as aria-label on the root span when showing initials (no image)', () => {
  ...
  it('does not set aria-label on the root span when alt is omitted in non-image state', () => {
    // No alt = purely decorative or initials-as-accessible-text use case.
    const { container } = render(<Avatar initials="JD" />);
    expect(container.querySelector('.mds-avatar')).not.toHaveAttribute(
      'aria-label',
    );
  });

  it('does not set derived aria-label on the root span when the image is showing', () => {
    // The <img alt> carries the accessible name; no redundant aria-label needed.
    const { container } = render(
      <Avatar
        imageSrc="https://example.com/photo.jpg"
        initials="JD"
    ...
    // aria-label must not be present (image alt is the label source)
    expect(container.querySelector('.mds-avatar')).not.toHaveAttribute(
      'aria-label',
    );
  });
