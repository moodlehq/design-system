...
export const Default: Story = {
  ...
  play: async ({ canvas }) => {
    // getByRole 'img' matches the inner <img alt="Jessica Doe"> directly
    const avatar = canvas.getByRole('img', { name: 'Jessica Doe' });
...
export const InitialsFallback: Story = {
  play: async ({ canvas }) => {
    // No image is present; the root span carries role="img" and aria-label
    // from the alt prop. The initials are visible but aria-hidden — they are
    // the visual content while the span provides the accessible name.
    const avatar = canvas.getByRole('img', { name: 'Jessica Doe' });
    await expect(avatar).toBeVisible();
    await expect(avatar).toHaveClass('mds-avatar');
  },
};
...
export const AllSizes: Story = {
  ...
  render: () => {
    ...
    const naStyle: React.CSSProperties = {
      textAlign: 'center',
      fontSize: '0.75rem',
      // #888 fails WCAG AA (3.5:1 on white). #595959 gives ~7:1.
      color: '#595959',
    };
