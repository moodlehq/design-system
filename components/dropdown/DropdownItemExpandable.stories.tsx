...
const meta = {
  ...
  argTypes: {
    ...
    children: {
      description: 'Dropdown item components rendered inside the submenu.',
      control: false,
      table: {
        type: {
          summary:
    ...
    open: {
      ...
      control: { type: 'boolean' },
      description:
        'Controlled submenu open state. Use alongside onOpenChange for fully controlled behavior.',
      table: {
        type: { summary: 'true | false' },
        defaultValue: { summary: 'false' },
      },
    ...
    placement: {
      ...
      table: {
        type: { summary: 'right-start | right-end | left-start | left-end' },
...
export const Default: Story = {
  render: function DefaultStory(args) {
    ...
    return (
      <DropdownMenu>
        <DropdownItemExpandable {...args} open={open} onOpenChange={setOpen}>
...
/**
 * Submenu placement variants — shows the expandable item with different placement
 * options: right-start, right-end, left-start, left-end. Use the controls to
 * select a placement and click or press Right Arrow to open the submenu.
 */
export const PlacementVariants: Story = {
  render: function PlacementStory(args) {
    const [open, setOpen] = useState(args.open ?? false);
...
export const RightToLeft: Story = {
  ...
  render: () => (
    <div dir="rtl">
      <DropdownMenu aria-label="القائمة">
        <DropdownItemExpandable label="توسيع" defaultOpen={false}>
          <DropdownItemAction label="إجراء فرعي ١" />
          <DropdownItemAction label="إجراء فرعي ٢" />
        </DropdownItemExpandable>
      </DropdownMenu>
    </div>
  ),
  play: async ({ canvas, userEvent }) => {
    const item = canvas.getByRole('menuitem', { name: 'توسيع' });
    // Item should start closed
    await expect(item).toHaveAttribute('aria-expanded', 'false');
    // In RTL, Left Arrow (not Right Arrow) is the open key on the button.
    item.focus();
    await userEvent.keyboard('{ArrowLeft}');
    await expect(item).toHaveAttribute('aria-expanded', 'true');
    // Confirm the submenu is open (two menus: parent + portaled submenu).
    await waitFor(() =>
      expect(document.querySelectorAll('[role="menu"]').length).toBe(2),
    );
    ...
    await userEvent.keyboard('{Escape}');
    await expect(item).toHaveAttribute('aria-expanded', 'false');
    // Open again with Left Arrow to test reopening
    await userEvent.keyboard('{ArrowLeft}');
    await expect(item).toHaveAttribute('aria-expanded', 'true');
    await waitFor(() =>
      expect(document.querySelectorAll('[role="menu"]').length).toBe(2),
