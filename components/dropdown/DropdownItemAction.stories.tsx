...
const meta = {
  title: 'Components/Dropdown/DropdownItemAction',
  ...
  argTypes: {
    ...
    description: {
      // Keep this control visible to avoid dead-end control combinations
      // (e.g. switching to danger then setting href).
      name: 'Secondary line (description)',
      control: { type: 'text' },
      description:
        'Optional secondary line below the label. This maps to the `description` prop; danger variant ignores it at runtime.',
      table: {
        type: { summary: 'string' },
        defaultValue: { summary: 'undefined' },
      },
...
/**
 * Danger variant matrix — default and disabled. Danger items don't support a
 * description (see DropdownItemAction.tsx), so no "with description" example
 * is shown here.
...
export const LinkVariants: Story = {
  ...
  play: async ({ canvas }) => {
    ...
    await expect(items[1].tagName).toBe('A');
    await expect(items[1]).toHaveAttribute('href', '/settings');
    await expect(items[3]).toHaveAttribute('aria-disabled', 'true');
    await expect(items[3]).not.toHaveAttribute('href');
  },
};
